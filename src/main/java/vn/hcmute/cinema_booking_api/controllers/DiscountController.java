...
public class DiscountController {
    ...
    public ResponseEntity<?> createDiscount(@RequestBody DiscountDTO discountDTO) {
        try {
            if(discountDTO == null){
                return ResponseEntity.badRequest().body(ApiResponse.error(400, "Invalid discount"));
            }
            discountService.creatDiscount(discountDTO);
            return ResponseEntity.ok(ApiResponse.success("Discount created"));
        }
        catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.badRequest().body(ApiResponse.error(400, e.getMessage()));
